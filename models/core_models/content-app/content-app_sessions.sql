...
with
    base as (
                ...
                cast(
                    replace(
                        case
                            when
                                regexp_contains(
                                    app_info_version, r'[0-9]+\.[0-9]+\.[0-9]+'
                                )
                            then
                                concat(
                                    lpad(
                                        split(app_info_version, '.')[offset (0)], 3, '0'
                                    ),
                                    '.',
                                    lpad(
                                        split(app_info_version, '.')[offset (1)], 3, '0'
                                    ),
                                    '.',
                                    lpad(
                                        regexp_replace(
                                            split(app_info_version, '.')[offset (2)],
                                            r'[^0-9](.*)',
                                            ''
                                        ),
                                        3,
        ...
        from {{ ref("content-app_events") }} c
        left join
            {{ ref("client_mapping_events_version") }} cm
            ...
            and cast(
                replace(
                    case
                        when
                            regexp_contains(
                                c.app_info_version, r'[0-9]+\.[0-9]+\.[0-9]+'
                            )
                        then
                            concat(
                                lpad(
                                    split(c.app_info_version, '.')[offset (0)], 3, '0'
                                ),
                                '.',
                                lpad(
                                    split(c.app_info_version, '.')[offset (1)], 3, '0'
                                ),
                                '.',
                                lpad(
                                    regexp_replace(
                                        split(c.app_info_version, '.')[offset (2)],
                                        r'[^0-9](.*)',
                                        ''
                                    ),
                                    3,
            ...
            between cm.version_from_num_long and cm.version_to_num_long
        left join
            {{ ref("event_config") }} ec
